...
/**
 * Compoñente InteractivePitch
 * Representa un campo de fútbol gaélico interactivo mediante SVG.
 * Permite ao usuario seleccionar unha posición (x, y) facendo clic no campo.
 * As coordenadas devoltas son porcentaxes (0-100) relativas ao tamaño do contedor.
 */
export default function InteractivePitch({ 
  ...
  // Estado para gardar o punto seleccionado actualmente
  const [selectedPoint, setSelectedPoint] = useState<{ x: number, y: number } | null>(
    initialX && initialY ? { x: initialX, y: initialY } : null
  );

  /**
   * Calcula o tamaño de fonte dinámico baseándose na lonxitude do nome do equipo.
   * Evita que nomes longos saian da área designada no céspede.
   */
  ...
  /**
   * Xestiona o clic no SVG, calcula as coordenadas relativas e actualiza o estado.
   */
  ...
  return (
    <div className="w-full aspect-[145/90] max-h-[300px] mx-auto relative bg-green-800 rounded border-2 border-gray-600 overflow-hidden shadow-inner">
      ...
      >
        ...
        {/* Indicador do punto seleccionado */}
        {selectedPoint && (
          <circle 
      ...
      {/* Mensaxe de instrución se non hai selección */}
      {!selectedPoint && (
